<?php
/**
 * JDIC - Program (Under Development) Feature Index
 */
$title = 'Programs - JDIC';
?>
<!DOCTYPE html>
<html lang="en">
    <?php include(__DIR__ . '/../../shared/layouts/head.php') ?>
    
    <body>
        <?php include(__DIR__ . '/../../shared/components/sidebar.php') ?>
        <?php include(__DIR__ . '/../../shared/components/navbar.php') ?>

        <section class="section-padding text-center">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <i class="fas fa-hammer fa-5x text-orange mb-4"></i>
                        <h1>Under Development</h1>
                        <p class="fs-4">This page is currently being updated. Please check back soon!</p>
                        <a href="index.php" class="theme-btn mt-4">Back to Home</a>
                    </div>
                </div>
            </div>
        </section>

        <?php include(__DIR__ . '/../../shared/components/footer.php') ?>
        <?php include(__DIR__ . '/../../shared/layouts/scripts.php') ?>
    </body>
</html>
